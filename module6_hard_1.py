...
class Figure:
    def __init__(self, __color: list[int], *__sides, filled=True):
...
class Circle(Figure):
    def __init__(self, __color: list[int], *__sides):
...
class Triangle(Figure):
    ...
    def get_square(self):
        ...
        area = math.sqrt(s * (s - self.__sides[0]) * (s - self.__sides[1]) *
                         (s - self.__sides[2]))
        return area
...
class Cube(Figure):
    def __init__(self, __color: list[int], *__sides):
        super().__init__(__color, __sides)
        self.sides_count = 12
        self.actual_sides = list(__sides) * 6
        #super().get_sides()
